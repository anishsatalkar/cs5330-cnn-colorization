...
def main():
    ...
    train_imagefolder = GrayscaleImageFolder(
        f'{config["train_val_split_dir"]}/train', train_transforms)
    ...
    validation_imagefolder = GrayscaleImageFolder(
        f'{config["train_val_split_dir"]}/val', validation_transforms)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            losses = Trainer.validate(
                validation_loader, model, criterion, save_images, epoch)
